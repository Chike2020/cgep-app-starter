...
# Requires: terraform >= 1.6, configured AWS credentials
...
run "gap01_s3_kms_encryption_enforced" {
  ...
  assert {
    condition     = aws_s3_bucket_server_side_encryption_configuration.uploads.rule[0].apply_server_side_encryption_by_default[0].kms_master_key_id == aws_kms_key.phi.arn
    error_message = "GAP-01: S3 uploads bucket must reference the PHI CMK, not an AWS-managed key"
  }

  assert {
    condition     = aws_s3_bucket_server_side_encryption_configuration.uploads.rule[0].bucket_key_enabled == true
...
run "gap02_dynamodb_kms_enforced" {
  ...
  assert {
    condition     = aws_dynamodb_table.intake_compliant.server_side_encryption[0].enabled == true
    error_message = "GAP-02: DynamoDB PHI table must have SSE enabled with a CMK"
  }

  assert {
    condition     = aws_dynamodb_table.intake_compliant.server_side_encryption[0].kms_key_arn == aws_kms_key.phi.arn
    error_message = "GAP-02: DynamoDB PHI table must reference the PHI CMK, not an AWS-owned key"
  }

  assert {
    condition     = aws_dynamodb_table.intake_compliant.point_in_time_recovery[0].enabled == true
...
run "gap03_s3_tls_policy_enforced" {
  ...
  assert {
    condition     = can(aws_s3_bucket_policy.uploads_tls_only.bucket)
    error_message = "GAP-03: S3 TLS-only bucket policy must exist"
  }
}
...
run "gap05_lambda_vpc_enforced" {
  ...
  assert {
    condition     = length(aws_lambda_function.intake_vpc.vpc_config[0].subnet_ids) > 0
    error_message = "GAP-05: Lambda function must be deployed in VPC private subnets"
  }

  assert {
    condition     = length(aws_lambda_function.intake_vpc.vpc_config[0].security_group_ids) > 0
...
run "evidence_vault_object_lock" {
  ...
  assert {
    condition     = aws_s3_bucket_object_lock_configuration.evidence_vault.rule[0].default_retention[0].mode == "COMPLIANCE"
    error_message = "Evidence vault must use COMPLIANCE mode Object Lock, not GOVERNANCE"
  }

  assert {
    condition     = aws_s3_bucket_object_lock_configuration.evidence_vault.rule[0].default_retention[0].days >= 90
...
run "gap07_iam_least_privilege_no_wildcard" {
  ...
  assert {
    condition     = can(aws_iam_role_policy.lambda_least_privilege.policy)
    error_message = "GAP-07: Least-privilege IAM policy must exist to replace the wildcard inline policy"
  }
}
...
run "monitoring_drift_detector_has_dlq" {
  ...
  assert {
    condition     = aws_lambda_function.drift_detector.dead_letter_config[0].target_arn == aws_sqs_queue.drift_detector_dlq.arn
